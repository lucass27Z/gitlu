...
export const ResizableLayout = ({
  ...
  maxWidth = 600,
  leftPannelClassName,
  rightPannelClassName,
}: {
  id: string;
  children: [React.ReactNode, React.ReactNode];
  minWidth?: number;
  maxWidth?: number;
  leftPannelClassName?: string;
  rightPannelClassName?: string;
}) => {
  ...
  return (
    ...
    >
      ...
      <div
        ref={leftRef}
        className={cn("relative border-r", leftPannelClassName)}
      >
        {children[0]}
      ...
      {/* Right */}
      <div className={cn(rightPannelClassName)} ref={rightRef}>
        {children[1]}
      </div>
    </div>
  );
